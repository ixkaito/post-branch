...
function kzpb_merge_post( $post, $request ) {
	...
	if ( $post->post_status != 'publish' || empty( $old_id ) ) {
		return;
	}
	...
	$new = array(
		...
		'post_date' => get_post( $old_id )->post_date,
		'post_date_gmt' => get_post( $old_id )->post_date_gmt,
		...
		'post_name' => get_post( $old_id )->post_name,
	...
	foreach ( (array) $keys as $key ) {
		...
		delete_post_meta( $old_id, $key );
		$values = get_post_custom_values($key, $id );
		foreach ( $values as $value ) {
			add_post_meta( $old_id, $key, maybe_unserialize ( $value ) );
		}
	}
	...
	if ( $attachments ) {
		foreach ( $attachments as $attachment ) {
			...
			$attachment_newi_d = wp_insert_post( $new );
	...
	foreach ( $taxonomies as $taxonomy ) {
		...
		wp_set_object_terms( $old_id, $terms, $taxonomy );
	}
}
